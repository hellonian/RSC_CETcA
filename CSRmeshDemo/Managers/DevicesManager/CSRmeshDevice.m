...
- (id)initDatabaseDevice:(NSDictionary *)deviceProperties {
    ...
    if (deviceProperties[kDEVICE_UUID]) {
        _uuid = [CBUUID UUIDWithString:deviceProperties[kDEVICE_UUID]];
    }
    ...
    if (deviceProperties[kDEVICE_SHORTNAME]) {
        _shortName = deviceProperties[kDEVICE_SHORTNAME];
    }
...
- (BOOL)timedSetLevel {
    ...
    NSNumber *brightness = @(desiredLevel);
    [[CSRDevicesManager sharedInstance] setLevel:_deviceId level:brightness];
